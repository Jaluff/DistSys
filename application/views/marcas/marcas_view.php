...
<div class="row">
 ...
 //    $user = $this->ion_auth->user()->row();
    ...
    <button class="btn btn-success" onclick="agregar_marca();"><i class="glyphicon glyphicon-plus"></i> Nueva marca</button>
...
<?php require_once(APPPATH . 'views/marcas/form_marca.php'); ?>
<script type="text/javascript">
    table = $('#clientes').DataTable({
        "processing": true, //Feature control the processing indicator.
        "serverSide": true, //Feature control DataTables' server-side processing mode.
        "createdRow": function ( row, data, index ) {
                console.log(data['marca_estado']);
                if( data['marca_estado'] === '0') {
                    $('td', row).eq(2).html('<h4><div class="label label-danger ">No activa</div></h4>');
                }
                if( data['marca_estado'] === '1') {
        ...
        "ajax": {
            "url": "<?php echo base_url() ?>marca/ajax_list",
            "type": "POST"
